...
$(document).ready(function () {
	...
	$("select[name='color_id']").change(function () {
		$(this).attr('selected', 'selected');;
	});
	...
	$('#form_add').submit(function (event) {
		...
		data.append('vendor_id', $('.vendors option:selected').val());
		data.append('sex', $("input[name='sex']:checked").val());
		data.append('category_id', $('.categories option:selected').val());
		$.ajax({
			url: 'product',
			type: 'POST',
			contentType: false,
	...
	$(document).on('click', '.edit', function () {
		...
		$.ajax({
			...
			success: function success(response) {
				...
				$('.categories option[value=id]').attr('selected', 'selected');
				$('.vendors option[value=id]').attr('selected', 'selected');
				...
				$('#form_edit').find('textarea[name="description"]').val(response[2].description);
	...
	$('#form_edit').submit(function (event) {
		...
		data.append('quantity', $('#form_edit').find('input[name="quantity"]').val());
		data.append('description', $('#form_edit').find('textarea[name="description"]').val());
		data.append('category_id', $('.categories option:selected').val());
		data.append('vendor_id', $('.vendors option:selected').val());
		data.append('sex', $("input[name='sex']:checked").val());
		$.ajax({
	...
	$(document).on('click', '#formadd', function (even) {
		...
		data.append('size_id', $('.sizes option:selected').val());
		data.append('color_id', $('.colors option:selected').val());
		$.ajax({
			url: 'productdetail',
			type: 'POST',
			contentType: false,
	...
	$(document).on('click', '.edit1', function () {
		...
		$.ajax({
			...
			success: function success(response) {
				...
				$('.sizes option[value='+response[2].size_id+']').attr('selected', 'selected');
				$('.colors option[value='+response[2].color_id+']').attr('selected', 'selected');
