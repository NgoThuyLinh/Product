						...
						<div class="row product-info-outer">
							<div class="col-xs-12 col-sm-5 col-md-6 col-lg-6">
								<div class="product-main-image">
									...
									<div class="product-main-image__item"><img class="img-product"  alt="" /></div>
								</div>
							</div>
							<div class="product-info col-xs-12 col-sm-7 col-md-6 col-lg-6">
								<div class="wrapper">
									<div class="product-info__sku pull-left">SKU: <strong id="code"></strong></div>
									<div class="product-info__availabilitu pull-right">Availability:   <strong class="color" id="availability"></strong></div>
								</div>
					...
					$.ajax({
						...
						success: function success(response) {
							...
							if (response[1]!= 'undefined') {
								...
								$('.img-product').attr("src", "{{asset('storage')}}"+'/'+response[0].thumbnail);
								$('#code').attr('data-id',response[0].id);
								$('#availability').text('In stock');
								$('#code').text(response[0].code);
								$('#title').text(response[0].name);
								$('.product-info__description__text').text(response[0].description);
								...
								$('.img-product').attr('src', '');
								$('.img-product').attr("src", "{{asset('storage')}}"+'/'+response.thumbnail);
								$('#code').attr('data-id',response.id);
